...
for s in subject_list:
    for r in run_list:
        wget = 'wget --no-check-certificate -P'
        location = data_path + '%s_filtered/sub'%(project_id) + s.zfill(3)\
           +'/BOLD/task001_run%s/'%(r.zfill(3)) + 'bold_filtered.nii.gz '
        link = base_url + 'sub%s/model/model001/task001_run' %(s.zfill(3))\
               + r.zfill(3) + '.feat/' + file_name
        txt_out.append((wget, location, link))

np_txt_out = np.array((txt_out))
np.savetxt(data_path + 'dwn_filtered_data_script.sh', np_txt_out, delimiter=" ", fmt="%s")
